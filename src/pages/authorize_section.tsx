...
export const AuthorizeSection = () => {
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            try {
                const sectionInstance: Section = await Section.createInstance(
                    parseInt(sectionId!)
                );
    ...
    useEffect(() => {
        if (!loading) {
            if (section) {
                const status = section.authorizeSection(String(pass));
                if (status) {
    ...
    }, [loading, toast, section, navigate, sectionId, pass]);
